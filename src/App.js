...
const App = () => {
  const [btc, setBtc] = React.useState(80);
  const [eth, setEth] = React.useState(30);
  const [link, setLink] = React.useState(60);
  ...
  return (
    <>
      ...
      <div className="instructions">Where do you think these tokens are going? Up or Down?</div>
      <div className="list">
        <Coin
          perc={btc}
          setPerc={setBtc}
      ...
      <Modal
        ...
        <div>{modalToken && abouts[abouts.findIndex((x) => x.token === modalToken)].about}</div>
      </Modal>
    </>
  );
};
